...
namespace delvers.Turns.Draw
{
	...
	public class DrawCards
	{
		...
		public void Draw()
		{
			if (this.player.Cards.Count >= 5)
			{
				GameLogger.LogFormat("{0} drew no cards because they have {1} already.", this.player.Name, this.player.Cards.Count);
				return;
			}

			while (this.player.Cards.Count < 5)
			{
				// draw a card from the bag
				var attackCard = new AttackCard(this.player, gameBoard, new LowestHpPlayer(), new RandomAmount());
				this.player.Cards.Add(attackCard);
